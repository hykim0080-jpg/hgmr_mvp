...
- 🚧 **심사 중 배포 동결 (2026-08-27 iOS 빌드 9 제출)** — `words.json` 을 배포하지 말 것. 앱이 실행할 때마다 원격 단어를 받아 가므로, **심사자가 보는 내용과 제출한 빌드가 어긋날 수 있다.** 단어 수정은 쌓아 두었다가 심사 결과가 난 뒤 한 번에 내보낸다.
  **단, `index.html` 웹 배포는 괜찮다** — 네이티브 앱은 «번들된» `index.html` 을 쓰고 원격에서 받아 오는 건 `words.json` 뿐이라, 웹 배포는 심사 중인 바이너리에 닿지 않는다. (`hgmr.co.kr` 은 지원 URL 이므로 깨진 상태로 두지만 말 것)
  **심사 결과가 나오면 이 항목을 지운다.**
...
- **🖥 넓은 화면(1100px 이상 · 마우스)은 2열이다.** `#quiz-box` 가 카드를 벗고 **격자**가 되고, 카드 옷은 새로 감싼 **`#quiz-main`** 이 입는다(왼쪽 열). `#inline-stats-container` 가 오른쪽 열이며, 답을 내기 «전»에는 `renderSidePanel()` 이 「오늘의 학습 · 방금 지나온 말」로 채우고, 답을 낸 뒤에는 **탭 없이 통계와 유의어·예문을 세로로 잇는다**. 좁은 화면은 지금까지의 1열·탭 그대로다.
  ...
  - 회귀 테스트: `node tests/desktop_two_col.mjs` (넓은 화면 2열 · 좁은 창 1열 · 홈에서 퀴즈 숨김)
