...
- **`--kb-height`는 키보드가 내려가는 도중 값에 오염될 수 있다** — 사파리는 키보드 전환 중 중간 높이로 `visualViewport.resize`를 여러 번 쏜다. 예전 코드는 `kb > 80`이기만 하면 그대로 반영해서, **닫히는 도중의 작은 값(예: 96px)이 마지막으로 남았다.** 통계 패널이 그 자리를 그대로 쓰기 때문에 패널이 짧게 떴다. 지금은 한 번 열린 동안의 **최대치만** 채택하고 완전히 닫히면 다시 잰다(`kbPeak`). 패널 높이는 `min(58vh, max(var(--kb-height), 380px))` — 키보드 자리보다 커도 된다(정답 공개 후라 위쪽은 비어 있다). 380px 은 탭·제목·표본 줄·표 머리·막대 5줄·다음 버튼이 다 들어가는 값이고, 58vh 는 작은 기기에서 문장까지 덮지 않게 하는 상한이다. **네이티브(Capacitor)는 플러그인이 정확한 높이를 주므로 이 경로를 타지 않는다 — 웹 전용 문제다**
