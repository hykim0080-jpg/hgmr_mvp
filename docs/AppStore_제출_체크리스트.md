...
> # ✅ 2026-08-27 심사 제출 완료 (빌드 9)
> 아래 표는 제출까지의 기록입니다. **재제출·업데이트 때 다시 쓰는 문서**이므로 지우지 마세요.
> 심사 결과 대응은 8절, 심사 중 주의사항은 10절.
...
| 4 | 연령 등급 설문 | ✅ 완료 — 결과 4+ | — |
| 5 | 스크린샷 | ✅ 촬영·업로드 완료 (6.9" 5컷) | — |
| 6 | 메타데이터 입력 | ✅ 완료 | — |
| 7 | App Privacy 선언 | ✅ 완료 (진단 제외) | — |
| 8 | 수출 규정(암호화) | ✅ 완료 — Info.plist 로 자동 처리 | — |
| 9 | 빌드 9 선택 후 제출 | ✅ **2026-08-27 제출 완료** | — |
...
#### 나중에 거래자로 바꾸는 절차 — 하루 만에 끝나지 않는다

바꾸는 것 자체는 언제든 됩니다(계정 단위 설정 후에도 앱마다 따로 지정 가능). 다만 **검증이 붙습니다.**

| 단계 | 내용 |
|---|---|
| 1 | 연락처 입력 — 개인은 **주소(또는 사서함) · 전화번호 · 이메일** |
| 2 | **이메일 2단계 인증** |
| 3 | **전화번호 2단계 인증** (불가하면 수동 검증 요청) |
| 4 | **주소 증빙 서류** — 사업자등록증 등 공식 서류. 사서함이면 그 주소와의 연관을 보이는 영수증·청구서 |
| 5 | 결제 계좌 정보 · EU 법 준수 인증 |

**4번이 병목입니다** — 사업자등록이 없으면 주소를 증빙할 공식 서류부터 마련해야 합니다.

> **순서 주의**: 인앱결제·광고를 «켜기 전에» 전환을 시작할 것. 켜고 나서 시작하면
> 검증이 끝날 때까지 사실과 다른 상태로 배포하게 됩니다.

**바꾸는 위치**
- 계정 전체 — Business 탭 → Agreements → Compliance → Digital Services Act
- 앱 하나만 — 앱 → 앱 정보 → App Store 규정 및 허가 → Digital Services Act 「편집」

권한은 Account Holder 또는 Admin (1인 계정이면 해당 없음).
>
